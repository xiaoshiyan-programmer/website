...
function print_board() {
  ...
  for (i = player_pos[0]-dif; i < player_pos[0]+dif+1; i++) {
    for (j = player_pos[1]-dif; j < player_pos[1]+dif+1; j++) {
      if(i<0 || j<0 || i>=n || j>=n){
        boardstr += "<span class='button' style='background-color:rgba(37, 37, 37, 0.685);'></span> "
      }
      else{
          if (board[i][j] == 0) {
  ...
  if (level > 100) {
    ...
    if (win == 2) {
      document.getElementById("win").innerHTML = "Sorry,but you can't move here!"
    }
  }
...
function choose() {
  var cmdstr = '<p>选择你的难度</p>\
    <button class="cmd" onclick="easy()">简单</button>\
    <button class="cmd" onclick="hard()">困难</button>\
    <button class="cmd" onclick="hell()">地狱</button>'
  document.getElementById("board").innerHTML = cmdstr
}
